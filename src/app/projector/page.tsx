...
function ProjectorContent() {
  ...
  const url = typeof window !== 'undefined' && accessCode ? 
    `${window.location.origin}/tap?access=${accessCode}&peer=${peerId}` : ''
    ...
    <div className="relative min-h-screen bg-black">
      <div className="fixed inset-0 z-0">
        <iframe
          ...
          src="https://www.youtube.com/embed/9wMflPIh3No?autoplay=1&mute=0&controls=0&modestbranding=1&rel=0&showinfo=0"
      ...
      <div className="fixed top-4 left-4 z-30">
        <div className="glass-card px-2 py-2 backdrop-blur-md bg-black/60">
          <Image src="/gdg-logo.png" alt="GDG" width={40} height={40} className="w-10 h-10 object-contain" />
        </div>
      </div>
      ...
      <div className="fixed top-4 right-4 z-30 flex gap-3">
        <button onClick={toggleFullscreen} className="glass-card p-2 backdrop-blur-md bg-black/60 hover:bg-white/10 transition">
          {isFullscreen ? <Minimize2 className="w-5 h-5" /> : <Maximize2 className="w-5 h-5" />}
        </button>
        <Link href="/admin" className="glass-card px-4 py-2 text-stadium-green hover:bg-white/10 transition">Exit</Link>
      </div>

      <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-30 flex gap-4">
        <button onClick={() => setShowQR(!showQR)} className={`glass-card px-4 py-2 backdrop-blur-md flex items-center gap-2 transition ${showQR ? 'bg-stadium-green text-black' : 'bg-black/60 text-white'}`}>
          <QrCode className="w-4 h-4" />
          {showQR ? 'Hide QR' : 'Show QR'}
        </button>
        <button onClick={() => setShowStats(!showStats)} className={`glass-card px-4 py-2 backdrop-blur-md flex items-center gap-2 transition ${showStats ? 'bg-stadium-green text-black' : 'bg-black/60 text-white'}`}>
      ...
      {showQR && (
        <div className="fixed top-24 right-4 z-20 animate-fade-in">
          <div className="glass-card p-5 text-center backdrop-blur-md bg-black/80 relative">
            <button onClick={() => setShowQR(false)} className="absolute top-2 right-2 p-1 hover:bg-white/10 rounded"><X className="w-4 h-4" /></button>
            ...
            {peerId ? (
              <>
                ...
                <div className="mt-3 flex items-center justify-center gap-2">
                  ...
                  <p className="text-xs text-gray-300">{isConnected ? 'Connected!' : 'Waiting...'}</p>
                </div>
              </>
            ) : (
              <div className="animate-pulse"><p>Loading QR...</p></div>
            )}
          </div>
        </div>
      )}
      ...
      {showStats && (
        <div className="fixed top-24 left-4 z-20 animate-fade-in">
          <div className="glass-card p-4 min-w-[200px] backdrop-blur-md bg-black/80 relative">
            <button onClick={() => setShowStats(false)} className="absolute top-2 right-2 p-1 hover:bg-white/10 rounded"><X className="w-4 h-4" /></button>
            ...
            {Object.values(reactionCount).every(v => v === 0) && (
              <p className="text-gray-500 text-sm text-center py-4">Waiting for reactions...</p>
            )}
          </div>
        </div>
      )}
